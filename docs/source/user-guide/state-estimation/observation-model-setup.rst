...
* **Observation Collection**, which is discussed on the :ref:`following page<observation_handling>`. These objects store the observations themselves and associated data used in a single estimation/covariance analysis in Tudat.

.. _observationTypes:

Defining observable model settings
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Tudat supports a diverse set of observables types.
The creation of an observation model is done in a similar manner as models used for the numerical propagation:
an object defining the settings of each observable model is created (of type :class:`~tudatpy.estimation.observable_models_setup.model_settings.ObservationModelSettings`),
which is then processed to create the actual observation model. The full list of available observable models
...
.. code-block:: python
  ...
  observation_model_settings = [
      model_settings.one_way_range(one_way_nno_mex_link_definition),
      model_settings.one_way_doppler_instantaneous(one_way_nno_mex_link_definition),
  ]
...
In addition to the minimal setup shown previously, you can for most types of models define settings for:

* **Biases:** A bias in Tudat is applied to the observable after its 'ideal' value computed from the environment is computed. You can find a list of settings for observation biases in our :ref:`API documentation <tudatpy:biases>`.
* **Light-time corrections:** When using an observable that involves the observation of one point/body in space by another, it is assumed that the signal travels at the speed of light in a straight line in Euclidean (e.g. flat) space. Deviations from the signal's ideal path (straight line at speed of light) due to relativistic, atmospheric, etc. effects may be defined by adding light-time correction settings, as listed in our :ref:`API documentation <tudatpy:light_time_corrections>`.
* **Light-time convergence settings:** Calculating the light time between two link ends requires the iterative solution of the light-time equation. The associated convergence settings can be created and modified using the :func:`~tudatpy.estimation.observable_models_setup.light_time_corrections.light_time_convergence_settings` function.
...
The above options are added to the calls of the observation model settings factory functions.
Below is an example where we have defined that the light-time calculation will take into account the first-order relativistic correction of the Sun, by using the :func:`~tudatpy.estimation.observable_models_setup.light_time_corrections.first_order_relativistic_light_time_correction` function.
For the range observable, we have defined an absolute bias of 1 cm (0.01 m) using the :func:`~tudatpy.estimation.observable_models_setup.biases.absolute_bias`, while leaving the Doppler observable unbiased.
...
.. code-block:: python
  ...
  range_bias_settings = biases.absolute_bias(0.01)
  ...
  observation_model_settings = [
      ...
      model_settings.one_way_doppler_instantaneous(
          one_way_nno_mex_link_definition,
          light_time_correction_settings=light_time_correction_settings,
      ),
  ]
...
Creating the observation simulators
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

The ``observation_model_settings`` in the above examples (a list of :class:`~tudatpy.estimation.observable_models_setup.model_settings.ObservationModelSettings`) is used to create the observation
...
* **Simulated observations**: In this case, the user must interact with the observation simulators (directly or indirectly), as discussed in more detail below
* **Real observations**: In this case, the observation simulators are created inside the estimation to generate the 'computed observations' from which the residuals are computed

The rest of this section is only relevant for using simulated observations in Tudat.
When using real observations, the observation simulators are automatically created upon instantiation of the :class:`~tudatpy.estimation.estimation_analysis.Estimator` object and stored inside the :attr:`~tudatpy.estimation.estimation_analysis.Estimator.observation_simulators` attribute.
...
In either case, the observation simulators is a list of objects derived from
:class:`~tudatpy.estimation.observable_models.observables_simulation.ObservationSimulator`, with a single object responsible for the simulation
of a single *type* of observable (*e.g.* one-way range, one-way Doppler, *etc.*). They are then used for simulating the actual observations to be used in the analysis, as described in :ref:`simulating_observations`.
...
You can extract these :attr:`~tudatpy.estimation.estimation_analysis.Estimator.observation_simulators` as follows, and later use them to :ref:`simulate observations <simulating_observations>`:
...
of the bodies, the propagation model, and the observation models can be made different between the two. The observation simulators to simulate the 'truth' observations are created directly, using the :func:`~tudatpy.estimation.observations_setup.observations_simulation_settings.create_observation_simulators` function:
...
.. code-block:: python
  ...
  observation_model_settings_truth = ...

  # Create observation simulators
  observation_simulators = create_observation_simulators(
      observation_model_settings_truth, bodies
  )     
...
to perform the estimation, one can then provide a different ``observation_model_settings_estimation`` to ensure a difference between the truth
...
one can choose different dynamical models for the states of the estimated bodies to ensure a difference in truth and estimation model.
