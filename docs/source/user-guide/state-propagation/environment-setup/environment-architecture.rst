...
* Each :class:`~tudatpy.dynamics.environment.Body` object contains a number of environment models. By default, each of these objects is initialized as a ``nullptr``, and is only assigned when specified during the creation of the body (:ref:`create_bodies_from_settings_first`), or when added after the creation of the body (:ref:`decorate_empty_body`).
