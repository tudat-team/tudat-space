...
In Tudat(Py) these parameters are referred to as estimated parameters, because of their primary application in state estimation problems.
%! can we rename Available Estimated Parameters to just Available Parameters? or something independent of estimation?

A description of how these parameters are to be defined and a comprehensive list of all available (estimated) parameters are linked below:
...
There are various types of ``DynamicsSimulator`` objects, each of which offers different functionality to the user.
The ``DynamicsSimulator`` objects for a simple single arc propagation of the state and for a combined propagation of state and variational equations are presented below.
...
In its simplest form, the ``DynamicsSimulator`` is used to obtain the single-arc propagated state of a single body, as shown in this example:
...
For a complete example of a perturbed single-arc propagation, please see the tutorial :ref:`_propagating_a_spacecraft_with_perturbations`.
    
=========================================================
Propagating System State & Variational Equations
=========================================================
    
To propagate the variational equations alongside the single-arc system state, the ``DynamicsSimulator`` object of type ``SingleArcVariationalEquationsSolver`` should be used.
Using the basic simulation setup (system of bodies, integrator settings, propagator settings) and the parameter settings for the variational equations, a variational equations solver can be set up.
