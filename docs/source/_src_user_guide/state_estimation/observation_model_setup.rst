...
Having defined the :ref:`link ends <linkEndSetup>`, you can now define and create the observation models. Below the general workflow for this is discussed. The observation models can play two roles in Tudat:P

* To generate simulated data inside Tudat, instead of loading real data.
* For use in a (least-squares) estimation loop, to fit input data (real or simulated) to the observations generated by an observation model
...
* Create an ::class:`~tudatpy.numerical_simulation.estimation_setup.Estimator` object (discussed further :ref:`here <perform_estimation>`), which creates the observation simulators automatically
  ...
  This option is typically used when the 'truth model' (e.g. the model used to generate simulated observations), and the 'estimation model', (e.g. the model used to perform the estimation) are identical. When using the above setup, identical observation simulators are used got the two cases.
  Also, this is the option that is used when performing an estimation from external data, since in this case the only observation simulator that is needed is the one inside the estimator.
...
* Create a set of observation simulators directly, using the :func:`~tudatpy.numerical_simulation.estimation_setup.create_observation_simulators` function:
  ...
  This option is typically used when simulating observations independently from an estimation, or when wishing to use a different observation model for the truth and estimation models.
